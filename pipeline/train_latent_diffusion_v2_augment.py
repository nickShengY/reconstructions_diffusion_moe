...
from models.diffusion.loss import   EDMLossSRAugment
...
from models.diffusion.augment import AugmentPipe
...
def main():
    ...
    edm_model = EDMPrecondSR(img_resolution=32, img_channels=24, cond_channels=24, sigma_data=0.06, augment_dim=9).to(args.device)
    load_checkpoint(args.load_ckpt, edm_model, args.device, strict=False)

    loss_fn = EDMLossSRAugment()
    optimizer = optim.AdamW(edm_model.parameters(), lr=args.lr)

    log_info(edm_model, args, optimizer)

    channel_config = ChannelConfig(channel_type='rayleigh', cuda=args.device=='cuda')
    pass_channel = Channel(channel_config)

    device = args.device

    augment_pipe = AugmentPipe( xflip=1e8, yflip=1, scale=1, rotate_frac=1, aniso=1, translate_frac=1)
    ...
    for epoch in range(args.epochs):
        ...
        for imgs in train_loader:
            imgs = imgs.to(device)
            imgs, aug_labels = augment_pipe(imgs)
            ...
            loss = loss_fn(net=edm_model, images=ground_truth_latent_img, x_cond=img_cond, labels=None, augment_labels=aug_labels).mean()
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        if epoch_num % 10 == 0 or epoch_num == args.epochs:
            save_path = os.path.join(args.output_dir, f"v2_edm_ffhq_latent_augment_222_{epoch_num}.pt")
