...
def main():
    ...
    start_epoch = 0
    if args.load_ckpt:
        ckpt = load_checkpoint(args.load_ckpt, edm_model, optimizer=optimizer, device=args.device, resume_opt=True, strict=False)
        if ckpt is not None:
    ...
    if start_epoch >= args.epochs:
        print(f"Checkpoint epoch {start_epoch} already reaches target epochs={args.epochs}; nothing to train.")
        return
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch_num % 10 == 0 or epoch_num == args.epochs:
            save_path = os.path.join(args.output_dir, f"v2_edm_ffhq_ckpt_latent_222_{epoch_num}.pt")
